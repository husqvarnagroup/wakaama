...
static read_server_t *prv_read_next_server(FILE *fd) {
    ...
    if (res == -1)
        goto error;
    if (readSrvP->id == 0 || readSrvP->uri == 0 ||
        (readSrvP->securityMode != LWM2M_SECURITY_MODE_NONE &&
...
error:
    if (serverP->securityData != NULL)
        lwm2m_data_free(serverP->securitySize, serverP->securityData);
    if (serverP->serverData != NULL)
        lwm2m_data_free(serverP->serverSize, serverP->serverData);
    lwm2m_free(serverP);
...
void bs_free_info(bs_info_t *infoP) {
    ...
    while (infoP->serverList != NULL) {
        ...
        if (targetP->securityData != NULL)
            lwm2m_data_free(targetP->securitySize, targetP->securityData);
        if (targetP->serverData != NULL)
            lwm2m_data_free(targetP->serverSize, targetP->serverData);
