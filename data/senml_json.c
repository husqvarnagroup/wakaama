...
static int prv_parseItem(const uint8_t *buffer, size_t bufferLen, _record_t *recordP, char *baseUri, time_t *baseTime,
    ...
    do {
        ...
        case 'b':
            if (tokenLen == 2 && buffer[index + tokenStart + 1] == 'n') {
                ...
                if (valueLen >= 3) {
                    if (valueLen == 3 && buffer[index + valueStart + 1] != '/')
                        return -1;
                    if (valueLen > URI_MAX_STRING_LEN)
                        return -1;
            ...
            } else if (tokenLen == 4 && buffer[index + tokenStart + 1] == 'v' &&
                       buffer[index + tokenStart + 2] == 'e' && buffer[index + tokenStart + 3] == 'r') {
                int64_t value;
                int res;
                if (bverSeen)
                    return -1;
        ...
        case 'v':
            if (tokenLen == 1) {
                ...
                if (0 == lwm2m_strncmp(JSON_TRUE_STRING, (char *)buffer + index + valueStart, valueLen)) {
                    ...
                    return -1;
                       ...
                       (buffer[index + tokenStart + 1] == 'd' || buffer[index + tokenStart + 1] == 's')) {
                if (recordP->value.type != LWM2M_TYPE_UNDEFINED)
                    return -1;
            ...
            } else if (tokenLen == 3 && buffer[index + tokenStart + 1] == 'l' &&
                       buffer[index + tokenStart + 2] == 'o') {
                if (recordP->value.type != LWM2M_TYPE_UNDEFINED)
                    return -1;
                /* Check for " around value */
                if (valueLen < 2 || buffer[index + valueStart] != '"' ||
                ...
                if (!utils_textToObjLink(buffer + index + valueStart + 1, valueLen - 2,
...
static bool prv_convertValue(const _record_t *recordP, lwm2m_data_t *targetP) {
    switch (recordP->value.type) {
    case LWM2M_TYPE_STRING:
        if (0 != recordP->value.value.asBuffer.length) {
            ...
            if (!string)
                return false;
            stringLen =
                json_unescapeString(string, recordP->value.value.asBuffer.buffer, recordP->value.value.asBuffer.length);
            if (stringLen) {
    ...
    case LWM2M_TYPE_OPAQUE:
        if (0 != recordP->value.value.asBuffer.length) {
            ...
            dataLength = utils_base64Decode((const char *)recordP->value.value.asBuffer.buffer,
                                            recordP->value.value.asBuffer.length, data, dataLength);
            if (dataLength) {
...
static int prv_convertRecord(const _record_t *recordArray, int count, lwm2m_data_t **dataP) {
    int index;
    int freeIndex;
    lwm2m_data_t *rootP;
    ...
    for (index = 0; index < count; index++) {
        ...
        if (recordArray[index].ids[1] != LWM2M_MAX_ID) {
            ...
            for (i = 1; i <= 2; i++) {
                if (recordArray[index].ids[i] == LWM2M_MAX_ID)
                    break;
                targetP = json_findDataItem(parentP->value.asChildren.array, parentP->value.asChildren.count,
        ...
        if (!prv_convertValue(recordArray + index, targetP))
            goto error;
    }
...
int senml_json_parse(const lwm2m_uri_t *uriP, const uint8_t *buffer, size_t bufferLen, lwm2m_data_t **dataP) {
    ...
    *dataP = NULL;
    ...
    while (recordIndex < count) {
        ...
        if (itemLen < 0)
            goto error;
        if (prv_parseItem(buffer + index + 1, itemLen - 2, recordArray + recordIndex, baseUri, &baseTime, &baseValue)) {
            goto error;
        }
        recordIndex++;
        index += itemLen - 1;
...
static int prv_serializeValue(const lwm2m_data_t *tlvP, uint8_t *buffer, size_t bufferLen) {
    ...
    case LWM2M_TYPE_CORE_LINK:
        ...
        res = json_escapeString(buffer + head, bufferLen - head, tlvP->value.asBuffer.buffer,
                                tlvP->value.asBuffer.length);
        if (res < tlvP->value.asBuffer.length)
            return -1;
        head += res;
    ...
    case LWM2M_TYPE_UNSIGNED_INTEGER: {
        ...
        if (0 == lwm2m_data_decode_uint(tlvP, &value))
            return -1;
    ...
    case LWM2M_TYPE_FLOAT: {
        ...
        if (!res)
            return -1;
        /* Error if inf or nan */
        if (buffer[head] != '-' && !isdigit(buffer[head]))
            return -1;
        if (res > 1 && buffer[head] == '-' && !isdigit(buffer[head + 1]))
            return -1;
        head += res;
    } break;
    ...
    case LWM2M_TYPE_BOOLEAN: {
        ...
        if (value) {
            if (bufferLen < JSON_ITEM_BOOL_SIZE + JSON_TRUE_STRING_SIZE)
                return -1;
            memcpy(buffer, JSON_ITEM_BOOL JSON_TRUE_STRING, JSON_ITEM_BOOL_SIZE + JSON_TRUE_STRING_SIZE);
            head = JSON_ITEM_BOOL_SIZE + JSON_TRUE_STRING_SIZE;
        } else {
            if (bufferLen < JSON_ITEM_BOOL_SIZE + JSON_FALSE_STRING_SIZE)
                return -1;
            memcpy(buffer, JSON_ITEM_BOOL JSON_FALSE_STRING, JSON_ITEM_BOOL_SIZE + JSON_FALSE_STRING_SIZE);
            head = JSON_ITEM_BOOL_SIZE + JSON_FALSE_STRING_SIZE;
        }
    } break;

    case LWM2M_TYPE_OPAQUE:
        if (bufferLen < JSON_ITEM_OPAQUE_BEGIN_SIZE)
            return -1;
        ...
        if (tlvP->value.asBuffer.length > 0) {
            res = utils_base64Encode(tlvP->value.asBuffer.buffer, tlvP->value.asBuffer.length, buffer + head,
                                     bufferLen - head);
            if (res < tlvP->value.asBuffer.length)
                return -1;
            head += res;
    ...
    case LWM2M_TYPE_OBJECT_LINK:
        if (bufferLen < JSON_ITEM_OBJECT_LINK_BEGIN_SIZE)
            return -1;
        memcpy(buffer, JSON_ITEM_OBJECT_LINK_BEGIN, JSON_ITEM_OBJECT_LINK_BEGIN_SIZE);
        head = JSON_ITEM_OBJECT_LINK_BEGIN_SIZE;

        res = utils_objLinkToText(tlvP->value.asObjLink.objectId, tlvP->value.asObjLink.objectInstanceId, buffer + head,
                                  bufferLen - head);
        if (!res)
            return -1;
        head += res;
...
static int prv_serializeData(const lwm2m_data_t *tlvP, const uint8_t *baseUriStr, size_t baseUriLen,
    ...
    case LWM2M_TYPE_OBJECT_INSTANCE: {
        ...
        for (index = 0; index < tlvP->value.asChildren.count; index++) {
            ...
            res = prv_serializeData(tlvP->value.asChildren.array + index, baseUriStr, baseUriLen, baseLevel, uriStr,
                                    uriLen, level, baseNameOutput, buffer + head, bufferLen - head);
            if (res < 0)
                return -1;
            head += res;
        }
    ...
    default:
        ...
        if (!*baseNameOutput && baseUriLen > 0) {
            if (bufferLen - head < baseUriLen + JSON_BN_HEADER_SIZE + 2)
                return -1;
        ...
        if (!baseUriLen || level > baseLevel) {
            if (bufferLen - head < JSON_ITEM_URI_SIZE)
                return -1;
...
int senml_json_serialize(const lwm2m_uri_t *uriP, int size, const lwm2m_data_t *tlvP, uint8_t **bufferP) {
    ...
    for (index = 0; index < num && head < PRV_JSON_BUFFER_SIZE; index++) {
        ...
        res = prv_serializeData(targetP + index, baseUriStr, baseUriLen, baseLevel, parentUriStr, parentUriLen,
                                rootLevel, &baseNameOutput, bufferJSON + head, PRV_JSON_BUFFER_SIZE - head);
        if (res < 0)
            return res;
        head += res;
    }
