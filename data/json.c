...
typedef enum { _TYPE_UNSET, _TYPE_FALSE, _TYPE_TRUE, _TYPE_FLOAT, _TYPE_STRING, _TYPE_OBJECT_LINK } _type;
...
static int prv_parseItem(const uint8_t *buffer, size_t bufferLen, _record_t *recordP) {
    size_t index;
...
static bool prv_convertValue(_record_t *recordP, lwm2m_data_t *targetP) {
    ...
    case _TYPE_TRUE:
        ...
        break;
    case _TYPE_FLOAT:
        if (1 != json_convertNumeric(recordP->value, recordP->valueLen, targetP))
            return false;
        break;
    ...
    case _TYPE_OBJECT_LINK:
        if (1 != utils_textToObjLink(recordP->value, recordP->valueLen, &targetP->value.asObjLink.objectId,
...
static int prv_serializeValue(lwm2m_data_t *tlvP, uint8_t *buffer, size_t bufferLen) {
    ...
    case LWM2M_TYPE_CORE_LINK:
        ...
        if (tlvP->value.asBuffer.length != 0 && res == 0)
            return -1;
        head += res;
    ...
    case LWM2M_TYPE_OBJECT_LINK:
        ...
        if (bufferLen - head < JSON_ITEM_OBJECT_LINK_END_SIZE)
            return -1;
...
static int prv_serializeData(lwm2m_data_t *tlvP, const uint8_t *parentUriStr, size_t parentUriLen, uint8_t *buffer,
                             size_t bufferLen) {
    int head;
    int res;
    ...
    case LWM2M_TYPE_MULTIPLE_RESOURCE: {
        ...
        for (index = 0; index < tlvP->value.asChildren.count; index++) {
            res = prv_serializeData(tlvP->value.asChildren.array + index, uriStr, uriLen, buffer + head,
                                    bufferLen - head);
            if (res < 0)
                return -1;
            head += res;
...
int json_serialize(lwm2m_uri_t *uriP, int size, lwm2m_data_t *tlvP, uint8_t **bufferP) {
    ...
    if (baseLevel >= URI_DEPTH_RESOURCE && rootLevel == baseLevel && baseUriLen > 1) {
        /* Remove the ID from the base name */
        while (baseUriLen > 1 && baseUriStr[baseUriLen - 1] != '/')
            baseUriLen--;
    ...
    for (index = 0; index < num && head < PRV_JSON_BUFFER_SIZE; index++) {
        ...
        res = prv_serializeData(targetP + index, parentUriStr, parentUriLen, bufferJSON + head,
