...
#if 0
static void cbor_test_data(const char * uriStr,
...
static void cbor_test_data_and_compare(const char * uriStr,
                        lwm2m_media_type_t format,
                        lwm2m_data_t * tlvP,
                        int size,
                        const char * id,
 ...
 */
static void cbor_test_raw(const char * uriStr,
                     ...
                     const char * id)
{
    ...
    cbor_test_data_and_compare(uriStr, format, tlvP, size, id, testBuf, testLen);
    ...
    {
        cbor_test_data(uriStr, LWM2M_CONTENT_TLV, tlvP, size, id);
    }
    else if (format == LWM2M_CONTENT_CBOR)
    {
        cbor_test_data(uriStr, LWM2M_CONTENT_TLV, tlvP, size, id);
    }
...
static void cbor_test_raw_expected(const char * uriStr,
                              const uint8_t * testBuf,
                              size_t testLen,
                              ...
                              const char * id)
{
    ...
    // Serialize to the same format and compare to the input buffer
    cbor_test_data_and_compare(uriStr, format, tlvP, size, id, expectBuf, expectLen);

    // Serialize to the other format respectively.
    if (format == LWM2M_CONTENT_TLV)
        cbor_test_data(uriStr, LWM2M_CONTENT_JSON, tlvP, size, id);
