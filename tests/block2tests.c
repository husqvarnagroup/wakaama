...
static void test_block2_simple_GET(void)
{
    lwm2m_block_data_t * blk = NULL;
    const uint16_t MID = 1234; // TODO this should be incremented for each block

    uint8_t * buffer0 = (uint8_t*)"0123";
    ...
    uint8_t status = coap_block2_handler(&blk, MID, buffer0, length0, blockSize, blockNum, blockMore, &resultBuffer, &resultLen);
    CU_ASSERT_PTR_NOT_NULL(blk)
    CU_ASSERT_EQUAL(status, COAP_231_CONTINUE);
    CU_ASSERT_PTR_NULL(resultBuffer)

    uint8_t * buffer1 = (uint8_t*)"4567";
    size_t length1 = strlen(buffer1);
    ++blockNum;
    status = coap_block2_handler(&blk, MID, buffer1, length1, blockSize, blockNum, blockMore, &resultBuffer, &resultLen);
    CU_ASSERT_EQUAL(status, COAP_231_CONTINUE);
    CU_ASSERT_PTR_NULL(resultBuffer)

    uint8_t * buffer2 = (uint8_t*)"89";
    size_t length2 = strlen(buffer2);
    ++blockNum;
    blockMore = false;
    status = coap_block2_handler(&blk, MID, buffer2, length2, blockSize, blockNum, blockMore, &resultBuffer, &resultLen);
...
static struct TestTable table[] = {
        { "test of test_block2_simple_GET()", test_block2_simple_GET },
        { NULL, NULL },
};
