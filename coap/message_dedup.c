...
void coap_cleanup_message_deduplication_step(coap_msg_dedup_t **message_dedup, const time_t current_time,
    ...
    while (message_dedup_check != NULL) {
        ...
        if (diff >= EXCHANGE_LIFETIME) {
            LOG_ARG("Message %d deduplication period ended", message_dedup_check->mid);
            ...
            LOG_ARG("Message %d check deduplication", message_dedup_check->mid);
            ...
            if (message_dedup_timeout < *timeout) {
                LOG_ARG("Message %d check again in %ds deduplication", message_dedup_check->mid, message_dedup_timeout);
...
bool coap_check_message_duplication(coap_msg_dedup_t **message_dedup, const uint16_t mid, const void *session,
    ...
    LOG_ARG("Register mid %d (session: %p) for deduplication check", mid, session);
...
void coap_deduplication_free(lwm2m_context_t *ctx) {
    LOG("Remove and free the whole message deduplication list");
